...
stages ranging from 1 to 4, without placement constraints figure for Spartan 7."""
import argparse
import sys
...
RO_TYPES: List[str] = ['LUTVar0', 'LUTVar5', 'WireVar', 'GateVar']
YLIM = (5e-18, 1e-9)
STAGE_MARKERS: List[str] = ['circle', 'tri_down', 'square', 'diamond']
TEXT_OFSETS: List[Tuple[float, float]] = [(1e-2, -7.5e-13), (2e-4, -4e-15),
                                          (1e-1, 2e-10), (0, -3e-11)]
...
for stages in range(1, 5):
    ...
    periods[1].append(([], []))
    file_name = join(DATA_FOLDER, 'intralut5_np',
                     f'all_configs_intralut5_np_coso_x0y0_stages{stages}.csv')
    with open(file_name, encoding='utf-8') as csv_file:
        csv_reader = csv.reader(csv_file, delimiter=',')
        next(csv_reader)
        for row in csv_reader:
...
graph_maker = g_m.GraphMaker('ranres_s7_no_placement.svg', figure_size=(1, 1),
                             folder_name='figures')
graph_maker.create_grid(size=(1, 1), marg_left=0.15)

ax = graph_maker.create_ax(0, 0, # pylint: disable=invalid-name
                           title=r'Range vs.\ resolution, no LP, no GP, Spartan 7',
