...
using Area Explore figure for Spartan 7."""
import argparse
import sys
import csv
...
DATA_FOLDER = join('measurements', 'no_placement_area_explore_s7')
...
store_data = s_d.StoreData(name='csc_s7_stage_length_area_explore')

cscs: Dict[str, Dict[int, List[float]]] = {}
if args.d:
    # Parse CSV files:
    for ro_type, stage_length in it.product(RO_TYPES, STAGE_LENGTHS):
        file_name = join(DATA_FOLDER, ro_type + '_np_ae',
                         (f'all_configs_{ro_type}_np_area_explore'
                          f'_coso_x0y0_stages{stage_length}.csv'))
...
graph_maker = g_m.GraphMaker('csc_s7_stage_length_area_explore.svg', figure_size=(1, 1),
                             folder_name='figures', figure_height_scale=0.85)
graph_maker.create_grid(size=(1, 1), marg_top=0.88, marg_bot=0.34, marg_left=0.11)
...
ax = graph_maker.create_ax(x_slice=0, y_slice=0, # pylint: disable=invalid-name
                           title='Obtainable $C$ values, Area Explore',
...
for stage_length in STAGE_LENGTHS:
    graph_maker.text(ax=ax, x=(len(RO_TYPES) + 1) * (stage_length - 0.5), y=Y_LIM[1] / 4,
