"""Generate obtained C values without specified GP and LP constraints figure for SmartFusion 2."""
import argparse
import sys
import csv
...
RO_NAMES: List[str] = ['GateVar', 'WireVar', 'LUTVar0', 'LUTVar3']
RO_TYPES: List[str] = ['muxnetwork', 'wireonly', 'intralut0', 'intralut3']
...
store_data = s_d.StoreData(name='csc_sf2_no_placement')
...
graph_maker = g_m.GraphMaker('csc_sf2_no_placement.svg', figure_size=(1, 1),
                             folder_name='figures', figure_height_scale=0.75)
graph_maker.create_grid(size=(1, 1), marg_left=0.11, marg_top=0.85, marg_bot=0.2)

ax = graph_maker.create_ax(x_slice=0, y_slice=0, # pylint: disable=invalid-name
                           title='Obtainable $C$ values without placement',
